...
public class GolemEnemy : MonoBehaviour
{
    ...
    [SerializeField] public GameObject miscellaneousItem;
    ...
    public void takeDamage(float damage)
    {
        ...
        print("Golem Health: " + this.health);
        anim.SetTrigger("Damaged");
        // if the enemy has 0 or less hp, destroy the enemy object
        if (this.health <= 0)
        {
            Instantiate(miscellaneousItem, transform.position, Quaternion.identity);    // drop item when enemy dies
            Die();
        }
    }
    ...
    {
        GetComponentInChildren<GolemDamageDealer>().BeginDealDmg();
    }

    public void EndDealDmg()
    {
        GetComponentInChildren<GolemDamageDealer>().EndDealDmg();
    }
    ...
    void Die()
    {
        ...
        Destroy(this.gameObject, 1);    // kills the game object after X seconds (set a timer to let the dying animation trigger)
        print("ENEMY DEAD");
    }
